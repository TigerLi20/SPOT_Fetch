...
class MosViz:
    ...
    def draw_observation(img, z, rx, ry, rth, r, size, color=(12, 12, 255)):
        ...
        for objid, objposes in z.objposes.items():
            if objposes is not None:
                lx, ly = objposes[0], objposes[1]
                cv2.circle(
    ...
    def on_init(self):
        try:
            ...
            print(f"DEBUG: Pygame window created, img_width={self.img_width}, img_height={self.img_height}")

            self._background = pygame.Surface(self._display_surf.get_size()).convert()
            self._clock = pygame.time.Clock()
            self._running = True
            print("DEBUG: MosViz initialized successfully")
            return True
        except Exception as e:
            print(f"ERROR: MosViz failed to initialize: {e}")
            return False
...
def unittest():
    ...
    worldstr = equip_sensors(worldmap, {robot: proxstr})
