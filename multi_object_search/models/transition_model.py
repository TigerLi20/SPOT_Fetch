...
class RobotTransitionModel(pomdp_py.TransitionModel):
    ...
    def if_move_by(cls, robot_id, state, action, dim, check_collision=True):
        ...
        if action.scheme == MotionAction.SCHEME_VW:
        ...
        elif action.scheme == MotionAction.SCHEME_XYTH:
    ...
    def argmax(self, state, action):
        ...
        if isinstance(action, MotionAction):
            ...
            newly_identified_targets = set()


            # Update found targets
            next_robot_state["objects_found"] = tuple(
                set(next_robot_state["objects_found"]) | newly_identified_targets
            )

            # Ensure termination does not assume predefined targets
        elif isinstance(action, FindAction):
            robot_pose = state.pose(self._robot_id)
            z = self._sensor.observe(robot_pose, state)
            
            # Note: Sensor is not an observation here!!!!!! 
